...
function [sens, y, sols] = senseq(pars,Init,NP)

[sols,y] = Model_wrapper(pars,Init,NP);

%Depending on form of y, the sensity matrix could be dymodel/dpars or 
%dr/dpars where r = ymodel - ydataivit

tspan=[0:0.01:NP]';
[sens] = diffjac(pars,@myfun,y,tspan,Init,NP);


function rout = myfun(pars,Init,NP)

[~,rout] = Model_wrapper(pars,Init,NP);

    
function [jac] = diffjac(x,f,f0,time,Init,NP)
...
for j = 1:n
    zz = zeros(n,1);
    zz(j) = 1;
    jac(:,j) = dirder(x,zz,f,f0,Init,NP);
end

function z = dirder(x,w,f,f0,Init,NP)
...
% [sols,y] = Model_solver(pars,Init,NP);
% [~,rout] = Model_solver(pars,Init,NP);
