...
%                 s2(i) = s2(i) + (norm(d{j,i}(ii,:),2) - mu(i)).^2;
    ...
    idxparTAA=find(morris_rank>avgmor);
    infparTAA=morris_rank(idxparTAA);
    infnamesTAA=par_names(idxparTAA);
    idxparTAA=[];
    for l=1:length(infnamesTAA)
        index=find(strcmp(Names,infnamesTAA{l}));
        idxparTAA=[idxparTAA index];
    end
    infnamesTAA=Names(idxparTAA);
    currentFile = sprintf('Inf_Pars_TAA.mat');
    save(currentFile,"idxparTAA","infnamesTAA","infparTAA");
    ...
    xlabel('Morris Mean');
    title('TAA Morris Results','FontSize',30);

    grid on;
    saveas(gcf,'TAA_Morris_Results.fig');

    % Plot ranking
    fig=figure(1000+ii);
    set(gcf,'Position',[200 50 1350 500]);
    ...
    xtickangle(-30);
    set(gca,'FontSize',20);
    ylabel('Morris Indices for ','FontSize',20);
    ylabel('Morris Indices for $\phi$','interpreter','latex',"FontSize",30);
    grid on;
    saveas(gcf,'TAA_Morris_Indices.fig');
    saveas(gcf,'TAA_Morris_Indices.svg');
